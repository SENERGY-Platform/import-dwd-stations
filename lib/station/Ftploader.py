...
DWD_MOST_RECENT_PATH = "climate_environment/CDC/observations_germany/climate/10_minutes/air_temperature/now/"
DWD_RECENT_PATH = "climate_environment/CDC/observations_germany/climate/10_minutes/air_temperature/recent/"
DWD_HISTORICAL_PATH = "climate_environment/CDC/observations_germany/climate/10_minutes/air_temperature/historical/"
DWD_RECENT_URL = "https://" + DWD_HOST + "/" + DWD_RECENT_PATH
DWD_HISTORICAL_URL = "https://" + DWD_HOST + "/" + DWD_HISTORICAL_PATH
DWD_MOST_RECENT_URL = "https://" + DWD_HOST + "/" + DWD_MOST_RECENT_PATH


def get_most_recent(station: Station) -> Optional[List[csv.DictReader]]:
    '''
    Downloads most recent data of a station

    :param station: DWD station
    :return: csv.DictReader to read the data or None if no data was available
    '''
    r = __get_with_url(DWD_MOST_RECENT_URL + "10minutenwerte_TU_" + station.station_id + "_now.zip")
    if r is None:
        logger.error("No most recent values for station with id " + station.station_id + ". Station still active?")
        return None
    return r
...
def get_recent(station: Station) -> Optional[List[csv.DictReader]]:
    ...
    :return: csv.DictReader to read the data or None if no data was available
    '''
    r = __get_with_url(DWD_RECENT_URL + "10minutenwerte_TU_" + station.station_id + "_akt.zip")
    if r is None:
...
def get_historical(stations: List[Station], callback: Callable[[Station, List[csv.DictReader]], any] = None) \
    ...
    for file in files:
        for station in stations:
            if file.count("TU_" + station.station_id):
